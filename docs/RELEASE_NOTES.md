...
This update adds persistent session pinning in a dedicated collapsible sidebar section, reversible local session archiving with a Settings-based restore manager, per-conversation message pinning, reversible local message archiving, and independent compact controls for Projects, pinned sessions, Sessions, and pinned messages. It also adds a visible-window switch for Seneschal's actual Playwright Brave bridge, so browser automation can be watched when needed. Changing visibility restarts only that bridge and applies on the next browser action.
